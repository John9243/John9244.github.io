...
var startPoint = {}, nowPoint;

bean.on(document, 'touchstart', function(event) {
...
bean.on(document, 'touchmove', function(event) {
    event.preventDefault();
    event.stopPropagation();

    var stY = localStorage.getItem("stY") ? parseInt(localStorage.getItem("stY")) : 0;
    var stX = localStorage.getItem("stX") ? parseInt(localStorage.getItem("stX")) : 0;
    ...
    if ((stX+100) < 200 && otk.x < 100 && otk.x > -100) {
      if (otk.y < stY) { 
        el.volume(el.volumeLevel + 0.05);
      } else if (otk.y > stY) { 
        el.volume(el.volumeLevel - 0.05);
      }    

      localStorage.setItem('stY', otk.y);
      localStorage.setItem('stX', otk.x);
    } 
}, false);

bean.on(document, 'mousewheel.fp', function(e) {
  var el = focused;
  if (!el) return;
  var vlc = el.conf.clip.vlc;
  if (vlc !== 1) return;
...
flowplayer(function(player, root) {
   ...
   player.playDisplayNone = function(i) {
      return player.resume2();
   };

   player.playDisplayNone3 = function(i) {
      return player.resume3();
   };
...
flowplayer(function(api, root) {
  ...
  bean.on(root, 'click', '.fp-qsel-menu a', function(ev) {
    var q = 0;
    if (ev.toElement != null && (ev.toElement.outerText == 'HD' || ev.toElement.outerText == 'hd')) {
      q = ev.toElement.parentElement.dataset.quality;
    } else q = ev.target.getAttribute('data-quality');
    
    api.quality(q);
    if (q > -1) {
      var l = ['360p', '480p', '720p', '1080p', '4K'];
      t = l[q];
    } else t = 'Auto';

    document.getElementsByClassName('fp-qsel')[0].innerHTML = t;
  });
  ...
  api.on('ready', function(_ev, _api, video) {
    ...
    // var tu = _api.video.height || 0;
    // console.log(tu);
    // var t = ({360: 0, 480: 1, 720: 2, 1080: 3, 2160: 4})[tu];
    // selectQuality(t, video.qualities);
  ...
  function createMenu(qualities) {
    controlbar.appendChild(common.createElement('strong', { className: 'fp-qsel' }, 'Auto'));
    var menu = common.createElement('div', { className: 'fp-menu fp-qsel-menu' }, '');
...
flowplayer(function(p, root) {
  ...
  var createSubtitleControl = function() {
    ...
    subtitleMenu = subtitleMenu || common.createElement('div', {className: 'fp-menu fp-subtitle-menu'}, '<strong>Субтитры</strong>');
    common.find('a', subtitleMenu).forEach(common.removeNode);
    subtitleMenu.appendChild(common.createElement('a', {'data-subtitle-index': -1}, 'Без субтитров'));
...
var LOADING_ROUNDED_OUTLINE = Buffer("PGRpdiBpZD0ibG9hZGluZyI+DQo8ZGl2IGNsYXNzPSJzcGlubmVyIj4NCiAgPGRpdiBjbGFzcz0iYm91bmNlMSI+PC9kaXY+DQogIDxkaXYgY2xhc3M9ImJvdW5jZTIiPjwvZGl2Pg0KICA8ZGl2IGNsYXNzPSJib3VuY2UzIj48L2Rpdj4NCjwvZGl2PjwvZGl2Pg==","base64")
...
function initializePlayer(element, opts, callback) {
  ...
  var root = element,
      ...
      var api = {
         ...
         resume2: function() {          
            document.getElementsByClassName('fp-play')[0].style.visibility = 'none!important';
            document.getElementsByClassName('fp-play')[0].style.opacity = '0!important';
            document.getElementsByClassName('fp-play-sharp-fill')[0].style.visibility = 'none!important';
            document.getElementsByClassName('fp-play-sharp-fill')[0].style.opacity = '0!important';
            ...
            document.getElementsByClassName('fp-play')[0].style.visibility = '';
            document.getElementsByClassName('fp-play')[0].style.opacity = '';
            document.getElementsByClassName('fp-play-sharp-fill')[0].style.visibility = '';
            document.getElementsByClassName('fp-play-sharp-fill')[0].style.opacity = '';
      ...
      if (!root.getAttribute('data-flowplayer-instance-id')) { // Only bind once
         ...
         api.on('boot', function() {
           ...
           api.splash = !is_ready;
            ...
            api.video.buffer = typeof buffered === 'number' ? buffered : typeof buffered.length != 'undefined' && buffered.length ? buffered[buffered.length - 1].end : 0;
         }).on("speed", function(e, api, val) {
...
flowplayer(function(o, e) {
    ...
    var t = function(o, e) {
        ...
        i = flowplayer.common,
        l = i.createElement,
        g = m.swf.indexOf("flowplayer.org") && m.e && e.getAttribute("data-origin"),
        s = g ? r(g) : i.hostname(),
        p = (document, m.key);
    if ("file:" == location.protocol && (s = "localhost"), o.load.ed = 1, m.hostname = s, m.origin = g || location.href, g && t(e, "is-embedded"), "string" == typeof p && (p = p.split(/,\s*/)), p && "function" == typeof c && c(p, s)) {
        if (m.logo) {
            var d = i.find(".fp-player", e)[0],
                u = m.logo.href || "",
                f = m.logo.src || m.logo,
                b = l("a", {
                    className: "fp-logo",
            ...
            g && (b.href = b.href || g), m.embed && m.embed.popup && (b.target = "_blank");
            var h = l("img", {
                src: f
            });
            b.appendChild(h), (d || e).appendChild(b)
        }
    } else {
        var b = l("a", {
          class: 'fp-logo userLogo'
        }),
            d = i.find(".fp-player", e)[0];
        (d || e).appendChild(b);
        var v = l("div", {
                className: "fp-context-menu fp-menu"
            }, '<strong style="text-align: center;">HDVB HTML player v1.0</strong><a data-clipboard-text="asfas" id="copyembedlink" >Копировать ссылку на видео</a><a id="copy_code">Копировать код вставки</a>'),
            k = window.location.href.indexOf("localhost");
        7 !== k && (d || e).appendChild(v), o.on("pause resume finish unload ready", function(o, r) {
            var n = -1;

            var url = "//" + r.conf.hostname + '/' + r.conf.conf.type + '/' + r.conf.conf.token + '/iframe';
          
            document.getElementById('copyembedlink').onclick = function () {
              copyTextToClipboard(url);
            }
            
            document.getElementById('copy_code').onclick = function () {
              copyTextToClipboard('<iframe src="'+url+'" width="610" height="370" frameborder="0" allowfullscreen></iframe>');
            }            
            ...
            var logo = typeof r.conf.clip.logo != 'undefined' ? r.conf.clip.logo.src : '';
            if (/pause|resume|ready/.test(o.type) && logo != '' || r.conf.clip.lip === true) {
               
                var _pos = r.conf.clip.logo.position;
                var _width = r.conf.clip.logo.width ? r.conf.clip.logo.width : false;
                var _height = r.conf.clip.logo.height ? r.conf.clip.logo.height : false;
                var _opacity = r.conf.clip.logo.opacity ? r.conf.clip.logo.opacity : false;
                ...
                if (_width !== false) p.width = _width; else p.width = '60px';
                if (_height !== false) p.height = _height; else p.height = '80px';
                if (_opacity !== false) p.opacity = _opacity; else p.opacity = '1';

                for (var i in p) p.hasOwnProperty(i) && (b.style[i] = p[i]);
                r.load.ed = a(b) && (7 === k || v.parentNode == e || v.parentNode == d), r.load.ed || r.pause()
