...
function checkStorage () {
    if ('localStorage' in window) {
        try {
            window.localStorage.testStorage = "test";
            return true;
        } catch(e) {}
    }
    return false;
...
function setCookie(a,b,c){c=c||{};var e=c.expires;if('number'==typeof e&&e){var f=new Date;f.setTime(f.getTime()+1e3*e),e=c.expires=f}c.domain = redomain;c.path   = '/';e&&e.toUTCString&&(c.expires=e.toUTCString()),b=encodeURIComponent(b);var g=a+'='+b;for(var h in c){g+='; '+h;var i=c[h];!0!==i&&(g+='='+i)}document.cookie=g}
function deleteCookie(a){setCookie(a,'',{expires:-1})}
// console.log(domain);
if (typeof cookie_name != 'undefined') {
    if (getCookie(cookie_name+'_'+domain) === undefined) setCookie(cookie_name+'_'+domain, window.location.protocol + '//' + domain + '/', {expires: 0});
}
...
var pauseBanner = function (api) 
{
    if (typeof adblock === 'undefined') return;
    if (domain == 'kinosweet.net' || domain == 'kino-vokrug.club') return;

    var self = this;
    this.api = api;

    if ((new RegExp(/test_page\=ok/g)).test(window.location.href) == true) {
        this.configs = {
            id: 'bannerBlock',

            key: '58025398ab118167ae4c1ae932c55939',
            class: '604c7625',
            script: 'https://aj1907.online/63c0d7d8.js'
        }
    } else if (in_array(user_id, [913]) && (new RegExp(/lordfilm\.cx/g)).test(domain) !== true) {
        this.configs = {
            id: 'bannerBlock',

            key: 'ae68d2e814adccc8f8e684f0c6c61cb1',
            class: '604c7625',
            script: 'https://aj1907.online/63c0d7d8.js'
        }
    } else if (in_array(user_id, [118, 432, 243, 1, 52, 51])) {
        this.configs = {
            id: 'bannerBlock',

            key: '5da16c1927946f72b715a90fd2af2d85',
            class: '604c7625',
            script: 'https://aj1907.online/63c0d7d8.js'
        }
    } else {
    ...
    this.fixSize = function() {
        var images = $('div#' + self.configs.id + ' ins img');
        images.each(setsize);

        function setsize() {
            var img = $(this),
                img_dom = img.get(0),
                container = img.parents('div#' + self.configs.id + ' ins');
            ...
            function resize() {
                var fpEngine            = document.getElementsByClassName('fp-engine')[0];
                var bannerPause         = document.getElementById('banner_pause');
    
                var window_width        = window.innerWidth;
                var window_height       = window.innerHeight;
                var bannerPauseWidth    = bannerPause.width;
                var bannerPauseHeight   = bannerPause.height;

                // alert('window_height - ' + window_height);
                // alert('window_height - ' + bannerPauseHeight);

                if (window_width == bannerPauseWidth && window_height >= bannerPauseHeight) 
                {
                    img.css({height: 'auto', width: '100%', left: 0, top: top +'px', position: 'absolute'});
                    var top = ( window_height - images.height() ) / 2;
                    img.css({top: top +'px'});
                    return;
                } 
                else if (window_width >= bannerPauseWidth) 
                {
                    img.css({height: '100%', width: 'auto'});
                    var left = ( images.width() - window_width ) / 2;
                    img.css({left: "-"+left+'px', top: '0', position: 'relative'});
                } 
                else 
                {
                    img.css({height: '100%', width: 'auto'});
                    var left = ( images.width() - window_width ) / 2;
                    img.css({left: "-"+left+'px', top: '-100%'});
                }                
            }
        }
    }

    this.genIns = function () 
    {
        var bannerBlock, insScript, insElement;

        bannerBlock             = document.createElement('div');
        bannerBlock.id          = self.configs.id;

        insScript               = document.createElement('script');
        insScript.src           = self.configs.script;
        insScript.setAttribute ('async', '');
        insScript.setAttribute ('defer', '');

        insElement              = document.createElement('ins');
        insElement.className    = self.configs.class;
        insElement.setAttribute ('data-key', self.configs.key);
        insElement.setAttribute ('data-cp-host', (domain.length > 0 ? domain : 'VASTHost_Fail') );
        
        bannerBlock.insertBefore( insScript, null );
        bannerBlock.insertBefore( insElement, null ); 

        $(window).on('resize', self.fixSize);

        this.insElement         = insElement;
        this.bannerBlock        = bannerBlock;

        var fpPlayer = document.getElementsByClassName('fp-ui')[0];
        fpPlayer.insertBefore(this.bannerBlock, null);  

        delete insElement;
        delete bannerBlock;
    }

    this.show = function (advObject) 
    {  
        if (advObject.actPid === false)
        {   
            if (self.api.paused === false) return false;

            var pause = true;

            $('.fp-player').click(function (a) 
            {
                if (a.target.tagName == 'IMG') return false;
                else if (a.target.className == 'img_banner_close_button')
                {
                    pause = false;
                }
            })

            // document.getElementsByClassName('fp-player')[0].onclick = function (a) {
            //     console.log(a, a.target.tagName);
            //     if (a.target.tagName == 'IMG') return false;
            // }
            ...
            if (pause === true)
            {
                setTimeout(function() 
                {
                    ...
                    {
                        var fpPause = document.getElementsByClassName('fp-pause')[0];
                            fpPause.classList.add('fp-visible');
                        self.bannerBlock.className = 'show';
                        self.fixSize ();
                    }
                }, 300);            
            }
    ...
    this.hide = function ()
    {
        self.bannerBlock.className = '';   
        var fpPause = document.getElementsByClassName('fp-pause')[0];
            fpPause.classList.remove('fp-visible');     
    }
...
var NativeAdv = function ( configs ) {
    ...
    this.readVastFile = function (vast_url, options, obj_vast){
        // if ((new RegExp(/my\.newseries\.cc\/205-agenty-schit\.html/g)).test(window.location.href) !== true)
        // {
            ...
            var urlSeparator = vast_url[0].indexOf('?') != -1 ? '&' : '?';
            
            vast_url = vast_url[0] +
            urlSeparator + 'cp.host=' + ( domain.length > 0 ? domain : 'VASTHost_Fail') +
            '&cp.ip=' + ( user_ip.length > 0 ? user_ip : 'VASTIP_Fail') +
            '&cp.adblock=' + (typeof adblock === 'undefined' ? 'true' : 'false') + '#' + vast_url[1];
        // }
        ...
        var VASTAdTagURI = xmlDoc.getElementsByTagName('VASTAdTagURI');
        if (VASTAdTagURI !== null && VASTAdTagURI.length > 0) {
            var uri = VASTAdTagURI[0].textContent;
            if (uri !== '') {
                if (!self.Vast.impression_tracked && !!impression && impression.length > 0) {
                    var img_2 = new Image();
                    img_2.src = impression[0].textContent;
                    self.Vast.impression_tracked = true;
                }
                return this.readVastFile(uri, {
                    'media_type' : 'video/mp4',
        ...
        for(var i=0;i<creative.length;i++){
            ...
            if (in_array(user_id, [52]))
            {
                obj_vast.time = '00:00:10';
            }

            if (in_array(user_id, [118, 432, 913, 243, 1, 51]))
            {
                obj_vast.time = '00:00:15';
            }

            if (in_array(user_id, [913]) && (new RegExp(/lordfilm\.cx/g)).test(domain) !== true)
            {
                obj_vast.time = '00:00:20';
            }
    ...
    this.videoPlayerTimeupdate = function(event) {
        if(self.Vast != '' && self.actPid === true) {
            ...
            if((current_time==0)){               
                if(!self.Vast.tracking_start_tracked){
                ...
                if (!self.Vast.impression_tracked && !!self.Vast.impression && self.Vast.impression.length > 0) {
                    var img_2 = new Image();
...
var AdvPlayer = function ( api ) {
    ...
    this.readVastFile = function (vast_url, options, obj_vast){
        ...
        if (isEmpty(obj_vast))
        {
            ...
            // vast_url[0] = 'https://aj1907.online/zKUlECTrnDQeeCQH_OnmIzwiwX3fo2-0n5Wz2dD9BWWTSlSIafahTTVjICwOpU8ajhPNktan63tZIagqRAR-NQLlUB8rNMPc';
            // console.log('vast2='+vast_url[0]);
            var urlSeparator = (decodeURIComponent(vast_url[0])).indexOf('?') != -1 ? '&' : '?';
            vast_url = vast_url[0] +
            urlSeparator + 'cp.host=' + ( domain.length > 0 ? domain : 'VASTHost_Fail') +
            '&cp.ip=' + ( user_ip.length > 0 ? user_ip : 'VASTIP_Fail') +
            '&cp.adblock=' + (typeof adblock === 'undefined' ? 'true' : 'false') + '#' + vast_url[1];
        ...
        //Get impression tag
        var impression = xmlDoc.getElementsByTagName("Impression");
        if(impression !== null && impression !== undefined && impression.length > 0){
            var impn = 0;
            if (typeof obj_vast.impression != 'object') {
                obj_vast.impression = [];
            } else {
                impn = obj_vast.impression.length;
            }

//             if (typeof impression[0]  === 'undefined') {
// console.log(impression);
//                 // self.videoPlayerEnded();
//                 return false;
//             }

            obj_vast.impression[impn] = impression[0].textContent;
        } 
        else {
            return false;
        }
                ...
                for(var j=0;j<creative_linear.length;j++){
                    ...
                    if(creative_linear_videoclicks!=null){
                        for(var k=0;k<creative_linear_videoclicks.length;k++){
                            ...
                            var creative_linear_videoclicks_clickthrough = creative_linear_videoclicks[k].getElementsByTagName("ClickThrough");
                   
                            if(creative_linear_videoclicks_clickthrough.length > 0){
                                creative_linear_videoclicks_clickthrough = creative_linear_videoclicks_clickthrough[0].childNodes[0].nodeValue;
                                  obj_vast.clickthrough_url = creative_linear_videoclicks_clickthrough;
                            }
        ...
        if (VASTAdTagURI !== null && VASTAdTagURI.length > 0) {
            var uri = VASTAdTagURI[0].textContent;
            if (uri !== '') {
                return this.readVastFile(uri, options, obj_vast); 
            }
        }
        return obj_vast;
    }
    ...
    this.videoPlayerTimeupdate = function(event) {
        ...
        if((current_time==0)){
            ...
            if (!self.Vast.impression_tracked && !!self.Vast.impression && self.Vast.impression.length > 0) {               
                self.Vast.impression_tracked = true;
                self.Vast.impression.forEach(function (imp) {
                    var img_2 = new Image();
                    img_2.src = imp;
                });
            }
    ...
    this.setSrcVideoPlayer = function ( src ) {
        ...
        // На засчитываем показ рекламы

        // Олегу на мидролле и 2ом преролле
        if (in_array(user_id, [913]) && self.prerollStart === true) {
            send_stat = false;
        }
        ...
        // на 2ом преролле
        if (self.prerollStart == true/* && hour >= 0 && hour < 7*/ && typeof self.midroll[1] != 'undefined' && self.midroll[1].time == 1) {
            send_stat = false;
        }

        // Отправка статистики о просмотре рекламы через VAST
        // Отправляем Start запись
        if (send_stat === true) {
            var clone_skip = window.location.protocol+'//'+window.location.host+'/vast/counter?event=start&id='+self.vast_id+'&ip='+user_ip+'&base='+(btoa(self.arrTrack))+'&host='+domain+'&token='+cookie_name+'&event_status='+event_status;
        ...
        if(self.Vast.tracking_skip_tracked === false){
            if(self.Vast.tracking_skip != null){
                ...
                for(var i=0;i<arrTrack.length;i++){
                    ...
                    // c 12 ночи до 7 утра, 2ой преролл
                    if (self.prerollStart == true && hour >= 0 && hour < 7 && typeof self.midroll[1] != 'undefined' && self.midroll[1].time == 1) {
                        send_stat = false;
                    }
                    ...
                    if (send_stat === true) {
                        clone_skip = window.location.protocol+'//'+window.location.host+'/vast/counter?event=skip&id='+self.vast_id+'&ip='+user_ip+'&base='+(btoa(arrTrack[i]))+'&host='+domain+'&token='+cookie_name+'&event_status='+event_status;
                        self.addPixel(clone_skip);
                    }
    ...
    this.skipTimer = function ( time ) {
        ...
        document.getElementById('close_button').style.display = 'none';
        document.getElementsByClassName('icon')[0].style.display = 'none';
        document.getElementsByClassName('skip_adv')[0].style.display = 'none';
        
        var getDurationInt = setInterval( 
            function ()
            {
                if (self.Video.duration != '' && !isNaN(self.Video.duration))
                {
                    ...
                    document.getElementsByClassName('skip_adv')[0].style.display = 'block';
                    document.getElementById('vast_time').innerText = time; time -- ;
    ...
    this.createPlayer = function (vast) {
        ...
        if( typeof adblock !== 'undefined' && (new RegExp(/kino\-fs\.pro/g)).test(domain) === true ) {
            var clone_skip = window.location.protocol+'//'+window.location.host+'/vast/counter?event=start&id=20&ip='+user_ip+'&base='+(btoa(self.arrTrack))+'&host='+domain+'&token='+cookie_name+'&event_status='+event_status;
            self.addPixel(clone_skip);

            setTimeout (function () {
                clone_skip = window.location.protocol+'//'+window.location.host+'/vast/counter?event=skip&id=20&ip='+user_ip+'&base='+(btoa(self.arrTrack))+'&host='+domain+'&token='+cookie_name+'&event_status='+event_status;
                self.addPixel(clone_skip);
            }, 11000);
        ...
        if ( typeof vast !== "undefined" ) {
            ...
            var show_banner = self.api.conf.end_tag_show_banner;
            if( typeof adblock !== 'undefined' && show_banner == true )
            {
                var imgBannerDisplayed = false;
                var bannerShowTime = parseInt(self.api.conf.end_tag_banner_show_time); //сколько показывать баннер
                var bannerCloseButtonTime = parseInt(self.api.conf.end_tag_banner_skip_time); // через сколько показывать кнопку закрыть
                var showCloseButton = true; // показывать кнопку закрыть?
                var startBannerTime = 0;

                var banner_time = parseInt(self.api.conf.end_tag_banner_time);
                var endTag = parseInt(self.api.conf.endTag);
                var domainParam = domain.length > 0 ? domain : 'VASTHost_Fail';

                //var timeToShowAds = Math.floor(self.api.video.duration - 10*60);
                console.log('endTag: ' + endTag);

                if( endTag && endTag != null && endTag != 0) // вычисление время показа баннера
                    timeToShowAds = endTag;
                else
                    timeToShowAds = Math.floor(self.api.video.duration - banner_time);

                self.api.on('progress', function () {
                    if( (self.api.video.time > timeToShowAds  && imgBannerDisplayed === false && document.getElementsByClassName('fp-eng')[0].style.display === 'none')){ // self.api.video.duration - длительность фильма
                        if( startBannerTime == 0 || startBannerTime + bannerShowTime > self.api.video.time) // проверка показа баннера в течении минуты
                        {
                            ...
                            {
                                if (in_array(user_id, [913]) && (new RegExp(/lordfilm\.cx/g)).test(domain) !== true) {
                                    document.getElementsByClassName('fp-player')[0].insertAdjacentHTML('beforeend','<div id="banner_before_end1" class="img_banner_block" style="position: absolute;top: 0%;left: 50%;transform: translate(-50%, 0%);z-index:9998; top: 47px;width: 80%;"><ins class="604c7625" data-key="fa14605f1ef460fe998b9f7ad5857b0b" data-cp-host="' + domainParam + '"></ins></div>');
                                } else {
                                    document.getElementsByClassName('fp-player')[0].insertAdjacentHTML('beforeend','<div id="banner_before_end1" class="img_banner_block" style="position: absolute;top: 0%;left: 50%;transform: translate(-50%, 0%);z-index:9998; top: 47px;width: 80%;"><ins class="604c7625" data-key="6c4661276978c9c87d15d0ba61646a8c" data-cp-host="' + domainParam + '"></ins></div>');
                                }
                    ...
                    if( startBannerTime!=0 && startBannerTime + bannerShowTime < self.api.video.time)
                        hideImageBanner()
...
var VpaidAd = function (url, cl, ap) {
  ...
  this.vpaidStart = function (id) {
      ...
      if ( country_code == 'ru' && city_name != 'moscow' && city_name != 'st petersburg' ) {
          ...
          var arr_vast_id = vast.split('-');
          var tmp_vast_id = arr_vast_id[arr_vast_id.length - 1];
          var clone_skip = window.location.protocol + '//' + window.location.host + '/vast/counter?event=start&id=' + tmp_vast_id + '&ip=' + user_ip + '&base=' + (btoa(self.url)) + '&host=' + domain + '&token=' + cookie_name + '&event_status=' + event_status;
          self.addPixel(clone_skip);

          setTimeout(function () {
              clone_skip = window.location.protocol + '//' + window.location.host + '/vast/counter?event=skip&id=' + tmp_vast_id + '&ip=' + user_ip + '&base=' + (btoa(self.url)) + '&host=' + domain + '&token=' + cookie_name + '&event_status=' + event_status;
              self.addPixel(clone_skip);
          }, 11000);
      }
      // Конец отправки статистики о просмотре рекламы
            ...
            function A() {
                ...
                f(this, A), this.writeDOM = function() {
                    ...
                    P(function(M, I) {
                        ...
                        var PIP = I.querySelector('.fp-pip');
                        M.on('ready', function() {
                            if (PIP !== null)
                                I.querySelector('.fp-controls').appendChild(PIP);
                        ...
                        //     var R = false;
                        // }
                        // console.log("cookie ", getCookie(O));
                        ...
                        M.one('ready', function() {
                            ...
                            (window.playFromCurrentPosition || (new RegExp(/resume=1/g)).test(window.location.href) === true) && !!R && M.seek(R.playBack)
                        });
                        ...
                        M.on('progress', function(Q, K, J) {
                            ...
                            if (2 <= Math.abs(J - U)) {
                                ...
                                setStorage(cookie_name, obj);

                                if ('parent' in window) {
                                    parent.postMessage(JSON.stringify({name: cookie_name, obj: obj}), '*');
                                }
                            }
                    ...
                    var H = u('#nativeplayer'),
                        ...
                        end_tag_banner_show_time = configs.end_tag_banner_show_time,
                        end_tag_banner_skip_time = configs.end_tag_banner_skip_time,
                        end_tag_show_banner = configs.end_tag_show_banner,
                        pause_banner_show = configs.pause_banner_show,
                        endTag = configs.endTag,
                        end_tag_banner_time = configs.end_tag_banner_time,
                        hbte = configs.hbte,
                        time = configs.time,
                        subtitles = [];
                    ...
                    if (!!J || !!Z) P();
                        ...
                        confs = {
                            ...
                            poster: poster,
                            end_tag_banner_show_time: end_tag_banner_show_time,
                            end_tag_banner_skip_time: end_tag_banner_skip_time,
                            end_tag_show_banner: end_tag_show_banner,
                            pause_banner_show: pause_banner_show,
                            end_tag_banner_time: end_tag_banner_time,
                            endTag: endTag,
                        ...
                        var flow = function () {
                            ...
                            api.on("ready", function () {
                                ...
                                // if (!(webOS||television())&&!pl) {
                                    ...
                                    // ) {
                                        AdvPlayerHDVB   = new AdvPlayer ( api );
                                    // } else {
                            ...
                            if(api.conf.pause_banner_show == true)
                            {
                                var pauseBanners    = new pauseBanner(api);
                                var pauseBannerBool = false;

                                api.on('pause', function () {
                                    pauseBanners.show(AdvPlayerHDVB);
                                });
